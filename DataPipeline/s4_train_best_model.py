...
def main():
    ...
    hpo_task_id = task.get_parameter(
        "General/hpo_task_id"
    )

    num_epochs = int(
        task.get_parameter("General/num_epochs")
    )
    ...
    s3_hpo_task = Task.get_task(
        task_id=hpo_task_id
    )

    best_hpo_result = (
        s3_hpo_task
        .artifacts["best_hpo_result"]
        .get()
    )

    best_train_task_id = (
        best_hpo_result["best_train_task_id"]
    )

    best_train_task = Task.get_task(
        task_id=best_train_task_id
    )

    best_model_name = best_train_task.get_parameter(
        "General/model_name"
    )

    best_batch_size = int(
        best_train_task.get_parameter(
            "General/batch_size"
        )
    )
    ...
    model_path = (
        f"{model_dir}/best_{best_model_name}.pth"
    )
    ...
    logger.info(
        f"Registered OutputModel ID: {output_model.id}"
    )
